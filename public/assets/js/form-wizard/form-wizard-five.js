...
(function($) {
    ...
    function onFinishCallback() {
        $('#staff-reg-form').submit();
    }

    $(document).on('click','input[name="same"]', function(){
        if ($(this).is(':checked')) {
            //autofill emergency contact area
            fillInputs();
        }else{
            //clear emergency contact if populated
            clearInputs();
        }
    });

    function getNxtKinValues()
    {
        const nxtOfkinObj = {
            name:          $('input[name="next_of_kin"]').val(),
            relationship:  $('input[name="rela_next_of_kin"]').val(),
            phone:         $('input[name="next_of_kin_phn"]').val(),
            email:         $('input[name="next_of_kin_email"]').val(),
        };
        return nxtOfkinObj;
    }
    ...
    function fillInputs()
    {
        let {name, relationship, phone, email} = getNxtKinValues();
        $('input[name="emergency_name"]').val(name);
        $('input[name="emergency_rela"]').val(relationship);
        $('input[name="emergency_phn"]').val(phone);
        $('input[name="emergency_email"]').val(email);
    }

    function clearInputs()
    {
        $('input[name="emergency_name"]').val('');
        $('input[name="emergency_rela"]').val('');
        $('input[name="emergency_phn"]').val('');
        $('input[name="emergency_email"]').val('');
    }

    $(document).on('click', '#more-competences', function(){
        let field = `<div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <div class="col-md-4">
                                       ...
                                       <input name="comp_doc[]" class="form-control" type="file">
                                    </div>
                                    <div class="col-md-4">
                        ...
                        <a href="" class="fa fa-times-circle" id="close-comp"></a>
                    </div>`;
        $(field).insertBefore($(this));
    });

    $(document).on('click', '#close-comp', function(e){
        e.preventDefault();
